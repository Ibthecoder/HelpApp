...
const handler = async (
  ...
  if (req.method === "POST") {
    try {
      ...
      if (error.message.includes("Booking not found") || error.message.includes("Review can only be submitted") || error.message.includes("Unauthorized") || error.message.includes("A review for this booking already exists")) {
